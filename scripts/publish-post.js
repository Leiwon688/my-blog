/**
 * 自动发布博客文章脚本
 * 直接操作 CloudBase 数据库发布文章
 */

import cloudbasePkg from '@cloudbase/node-sdk';
const cloudbase = cloudbasePkg.default || cloudbasePkg;
...
async function publishPost() {
  try {
    ...
    // 使用未登录模式（需要在 CloudBase 控制台开启"允许未登录访问"）
    console.log('CloudBase 初始化成功');

    // 检查文章是否已存在
    const existRes = await db.collection('posts').where({ id: todayPost.id }).get();
    
    if (existRes.data && existRes.data.length > 0) {
      // 更新文章
      await db.collection('posts').where({ id: todayPost.id }).update({
        ...todayPost,
        updatedAt: Date.now(),
      });
      console.log('文章已更新:', todayPost.title);
    } else {
      // 新建文章
      ...
      console.log('文章已发布:', todayPost.title);
    }

    // 更新标签
    const tagsRes = await db.collection('tags').get();
    const allTags = new Set(todayPost.tags);
    
    if (tagsRes.data && tagsRes.data.length > 0) {
      const existingTags = tagsRes.data[0].tags || [];
      existingTags.forEach(tag => allTags.add(tag));
      
      await db.collection('tags').where({ _id: tagsRes.data[0]._id }).update({
